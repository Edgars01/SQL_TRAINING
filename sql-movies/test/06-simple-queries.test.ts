...
describe("Simple Queries", () => {
  ...
  it(
    ...
    async done => {
      const query = `SELECT ROUND(SUM(budget_adjusted),2) , ROUND(SUM(revenue_adjusted),2) as total_revenue 
      FROM movies
      `;
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(budget) as count
      FROM movies 
  ...
  it(
    ...
    async done => {
      const query = `SELECT COUNT(id) as count, strftime('%Y', release_date) as year
      FROM movies
      GROUP by year
      ORDER by year DESC
      `;
  ...
  it(
    ...
    async done => {
      const query = `SELECT count(movie_id) as count, strftime('%m', time_created) as month
      from MOVIE_RATINGS 
